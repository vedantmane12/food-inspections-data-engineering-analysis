...
# # ============================================================================
# # GOLD ZONE - Fact: Food Inspections (Inspection Level)
# # Grain: One row per inspection (not per violation)
# # ============================================================================

# @dlt.table(
#     name="fact_food_inspections",
#     comment="Gold layer - Fact table at inspection grain"
# )
# def fact_food_inspections():
#     """
#     Fact table: Inspection-level facts with foreign keys to dimensions
#     Grain: One row per unique inspection
#     Measures: scores, violation counts
#     """    
#     # Read from Silver transformed (streaming)
#     df = dlt.read_stream("silver_food_inspections_transformed")
    
#     # Create inspection business key
...
#     df = df.dropDuplicates(["inspection_business_key"])
    
#     # Generate inspection_key (surrogate key - BIGINT)
#     df = df.withColumn("inspection_key",
#         abs(hash(col("inspection_business_key"))) % 9223372036854775807)
    
#     # Create date_key (YYYYMMDD format for joining to dim_date)
...
#     # Create restaurant_id for joining (will join to current version)
#     df = df.withColumn("restaurant_id",
#         concat(col("license_number"), lit("-"), col("city")))
...
#         # Foreign key placeholders (will be resolved in BI tool or queries)
#         "restaurant_id",              # Join to dim_restaurant WHERE is_current=TRUE
#         "date_key",                   # Join to dim_date
        
#         # Degenerate dimensions (inspection-level attributes)
#         "inspection_id",
#         "license_number",
#         "inspection_type",
#         "results_std",
...
#         # Measures (facts - numeric values for aggregation)
...
#         # Dimensional attributes (can also join to dimensions)
#         "city",                       # Source city
